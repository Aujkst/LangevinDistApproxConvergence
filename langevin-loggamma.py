...
class LangevinAlgoSampler(object):
    ...
    def run(self):
        ...
        return np.array(samples[200:]), np.array(grads[200:])
...
if __name__ == '__main__':
    ...
    step_size = 1e-1
    max_itr = 1e5
    t = np.arange(step_size, (max_itr + 1.0) * step_size, step_size)[200:]
    ...
    grads = {}
    true_result = {}
    for name, (target_dist, sample_gen_func, _dist) in params_dists.items():
        ...
        samples[name] = sample.T.squeeze()
        grads[name] = grad.T.squeeze()
        if sample_gen_func is not None:
            samples[name] = sample_gen_func(samples[name].T.squeeze())
